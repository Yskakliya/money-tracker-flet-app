...
def get_profile_view(page: ft.Page, user_name="User"):
    ...
    def goal_card(g):
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Text(g["goal_name"], color="white", weight="bold",
                            size=14, expand=True),
                    pct_text,
                ]),
                ft.Stack([
    ...
    def add_goal(e):
        ...
        try:
            ...
            cursor = db.cursor(dictionary=True)
            cursor.execute(
                "INSERT INTO user_goals (user_name, goal_name, goal_price, saved_amount) "
                "VALUES (%s,%s,%s,%s)",
    ...
    guide_tab = ft.Column([
        ...
        guide_step(1, "Create your Budgets",
            'Go to "Budget" → enter a category name (e.g. "Food", "Rent") and set a '
            'spending limit → click "+ Add".',
            ft.icons.ACCOUNT_BALANCE_WALLET_OUTLINED, PRIMARY),

        guide_step(2, "Log Expenses",
            'Use "Add Expense" in the sidebar OR click any day in the Calendar to add '
            'a transaction to that day.',
            ft.icons.ADD_CIRCLE_OUTLINE, BLUE),

        guide_step(3, "Track in Calendar",
            'Open "Calendar" to see your spending day by day. Each day shows the amount '
            'and category label.',
            ft.icons.CALENDAR_MONTH_OUTLINED, PRIMARY),

        guide_step(4, "Monitor Dashboard",
            '"Dashboard" shows your monthly summary, daily chart, budget alerts, and '
            'top spending category.',
            ft.icons.DASHBOARD_OUTLINED, AMBER),

        guide_step(5, "View History",
            '"History" lists all transactions. Filter by category, search, or sort by '
            'date and amount.',
            ft.icons.RECEIPT_LONG_OUTLINED, BLUE),

        guide_step(6, "Set Savings Goals",
            'In "Profile → Goals" add a goal (e.g. "New Phone $800") and track how '
            "much you've saved.",
            ft.icons.FLAG_OUTLINED, PRIMARY),

        guide_step(7, "Export your Data",
            'Go to "Export" and download a full Excel or PDF report of all your '
        ...
        tip_card(
            "The Dashboard projects your month-end spending based on your daily average.",
            ft.icons.TRENDING_UP_OUTLINED, BLUE,
        ),
        tip_card(
            "You can delete any transaction in History — it will update the budget automatically.",
    ...
    for idx, (label, icon) in enumerate(tab_labels):
        btn = ft.Container(
            ...
            border=ft.border.all(
                1, ft.colors.with_opacity(0.4, PRIMARY) if idx == 0 else BORDER2
            ),
                    ...
                    content=ft.Column([
                        ...
                        ft.Row([
                            ...
                            ft.Column([
                                ...
                                ft.Text("Personal Finance Profile",
                                        color=TEXT_GREY, size=12),
